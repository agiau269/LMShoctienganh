...
# Tự bổ sung cột video nếu database cũ đã tồn tại
def migrate_database():
    con = connect()
    columns = [row["name"] for row in con.execute("PRAGMA table_info(lessons)").fetchall()]
    if "video_file" not in columns:
        con.execute("ALTER TABLE lessons ADD COLUMN video_file TEXT")
    if "video_name" not in columns:
        con.execute("ALTER TABLE lessons ADD COLUMN video_name TEXT")
    con.commit()
    con.close()
    ...
    for lesson in lessons:
        ...
        with st.expander("Xem nội dung bài giảng"):
            if lesson["video_file"]:
                video_path = Path(lesson["video_file"])
                if video_path.exists():
                    st.video(str(video_path))
                    st.caption(f"Video: {lesson["video_name"] or video_path.name}")
                else:
                    st.warning("Không tìm thấy file video trên hệ thống.")
...
def create_lesson():
    ...
    with st.form("create_lesson"):
        title = st.text_input("Tên bài giảng")
...
"""
        ...
        st.caption("Bạn có thể tải video trực tiếp từ máy tính lên website.")
        video = st.file_uploader(
            "Chọn video",
            type=["mp4", "mov", "webm", "m4v"],
            help="Khuyến nghị MP4 để học sinh xem ổn định nhất."
        )

        if video is not None:
            st.video(video)
            st.caption(f"Video đã chọn: {video.name}")
    ...
    if submit:
        ...
        if video is not None:
            safe_name = Path(video.name).name.replace(" ", "_")
            unique_name = f"{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}_{safe_name}"
            video_path = VIDEO_DIR / unique_name
        ...
        execute("""
            INSERT INTO lessons
            (title,description,level,category,content,resource_url,video_file,video_name,created_at)
            VALUES(?,?,?,?,?,?,?,?,?)
        """, (
            title,
            description,
...
def open_student_lesson(lesson_id):
    ...
    if lesson["video_file"]:
        ...
        if video_path.exists():
            ...
            st.warning("Video của bài giảng hiện không khả dụng.")
